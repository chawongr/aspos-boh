...
const HqSyncDownPage = () => {
  ...
  const [editData, setEditData] = useState<HqSyncDown | null>(null);
  ...
  const fetchHqSyncDown = async (params: TDataGridRequestParams) => {
    try {
      ...
      toast.error("Error fetching HqSyncDown");
      return { data: [], totalCount: 0 };
    }
  };

  const handleEdit = (rowData: HqSyncDown) => {
    setEditData(rowData);
  ...
  const handleDelete = async (code: string) => {
    toast("Are you sure you want to delete this HqSyncDown?", {
      action: (
        <button
          onClick={async () => {
            try {
              await deleteArea(code);
              toast.success("HqSyncDown deleted successfully!");
              setRefreshKey(prev => prev + 1);
            } catch (error) {
              toast.error("Error deleting HqSyncDown.");
            }
          }}
  ...
  const handleSave = async () => {
    try {
      if (editData) {
        ...
        toast.success("HqSyncDown updated successfully!");
      } else {
        await addArea(formData.code, formData.name);
        toast.success("HqSyncDown added successfully!");
      }
      setShowAddForm(false);
      setEditData(null);
      setFormData({ code: '', name: '' });
    } catch (error) {
      toast.error("Failed to save HqSyncDown.");
    }
  };
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        id: 'menuitem',
        header: ({ column }) => <DataGridColumnHeader title="Menu Item" column={column} />,
        enableSorting: true,
        cell: ({ row }) => (
        ...
        {showAddForm ? (
          <div className="card">
            <div className="card-header" id="webhooks">
              <h3 className="card-title">{editData ? 'Edit Hq Sync Down' : 'New Hq Sync Down'}</h3>
            </div>
          ...
          <div className="overflow-x-auto ">
            <div className="card mb-7">
              <div className="card-header" id="webhooks">
                <h3 className="card-title">Select Hq Sync Down</h3>
              </div>
              <div className="card-body grid gap-5">
                <div className="items-center flex-wrap lg:flex-nowrap gap-2.5">
                  <span className="form-label max-w-32 w-full">
              ...
              <div className="card-footer justify-end">
                <button className="loadBtn text-sm h-8 bg-blue-500 hover:bg-blue-600 text-white flex justify-center items-center gap-x-1" onClick={handleSave}>
                  <div className='text-base mt-[1px]' style={{ transform: 'scaleX(-1)' }}>
