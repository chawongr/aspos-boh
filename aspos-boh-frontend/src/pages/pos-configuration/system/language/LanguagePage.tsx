...
import { addLanguage, deleteLanguage, editLanguage } from '@/pages/pos-configuration/system/Service';
...
const LanguagePage = () => {
  ...
  const [editData, setEditData] = useState<Language | null>(null);
  const [formData, setFormData] = useState({ code: '', language1: '' });
  ...
  const fetchLanguage = async (params: TDataGridRequestParams) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/config/language?${queryParams.toString()}`, {
        headers: {
  ...
  const handleEdit = (rowData: Language) => {
    setEditData(rowData);
    setFormData({ code: rowData.code, language1: rowData.language1 });
    setShowAddForm(true);
  };
            ...
            <ToolbarHeading>
              ...
              <ToolbarDescription>
                <div className="flex items-center flex-wrap gap-1.5 font-medium">
                  <span className="text-md text-gray-600">Language Management</span>
                </div>
              </ToolbarDescription>
            </ToolbarHeading>
            <ToolbarActions>
        ...
        {showAddForm ? (
          <div className="card">
            <div className="card-header" id="webhooks">
              <h3 className="card-title">{editData ? 'Edit Language' : 'New Language'}</h3>
            </div>
            <div className="card-body grid gap-5">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="items-center flex-wrap lg:flex-nowrap gap-2.5">
                  <span className="form-label max-w-32 w-full">
                    Language {!formData.language1 && <span className="text-red-500">*</span>}
                  </span>
                  <div className="grow min-w-24">
                    <input
                      className="input w-full"
            ...
            <div className="card-footer justify-end">
              ...
              <button
                ...
                onClick={() => { setShowAddForm(false); setEditData(null); setFormData({ code: '', language1: '' }); }}
              >
          ...
          <div className="overflow-x-auto ">
            <DataGrid
              ...
              columns={columns}
              serverSide={true}
              onFetchData={fetchLanguage}
              rowSelection={true}
              pagination={{ size: pageSize }}
              toolbar={
                <div className="card-header flex-wrap gap-2 border-b-0 px-5">
                  <h3 className="card-title font-medium text-sm">
                    Showing {count} of {totalCount} languages
                  </h3>
