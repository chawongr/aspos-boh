...
import { addRegion, deleteRegion, editRegion } from '@/pages/pos-configuration/system/Service';
...
const RegionPage = () => {
  ...
  const fetchRegion = async (params: TDataGridRequestParams) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/location/region?${queryParams.toString()}`, {
        headers: {
  ...
  const handleEdit = (rowData: Region) => {
    setEditData(rowData);
  ...
  const handleSave = async () => {
    try {
      if (editData) {
        await editRegion(formData.code, formData.name);
        toast.success("Region updated successfully!");
      } else {
        await addRegion(formData.code, formData.name);
            ...
            <ToolbarHeading>
              ...
              <ToolbarDescription>
                <div className="flex items-center flex-wrap gap-1.5 font-medium">
                  <span className="text-md text-gray-600">Region Management</span>
                </div>
              </ToolbarDescription>
            </ToolbarHeading>
            <ToolbarActions>
        ...
        {showAddForm ? (
          <div className="card">
            <div className="card-header" id="webhooks">
              <h3 className="card-title">{editData ? 'Edit Region' : 'New Region'}</h3>
            </div>
          ...
          <div className="overflow-x-auto ">
            <DataGrid
              ...
              columns={columns}
              serverSide={true}
              onFetchData={fetchRegion}
              rowSelection={true}
              pagination={{ size: pageSize }}
              toolbar={
                <div className="card-header flex-wrap gap-2 border-b-0 px-5">
                  <h3 className="card-title font-medium text-sm">
                    Showing {count} of {totalCount} regions
                  </h3>
