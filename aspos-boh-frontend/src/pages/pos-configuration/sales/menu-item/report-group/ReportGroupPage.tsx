...
import { addReportGroup, deleteReportGroup, editReportGroup } from '@/pages/pos-configuration/sales/Service';
...
const ReportGroupPage = () => {
  ...
  const [editData, setEditData] = useState<StoreGroup | null>(null);
  ...
  const fetchReportGroup = async (params: TDataGridRequestParams) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/config/menu/report-group?${queryParams.toString()}`, {
        headers: {
      ...
      toast.error("Error fetching report group");
      return { data: [], totalCount: 0 };
    }
  };

  const handleEdit = (rowData: StoreGroup) => {
    setEditData(rowData);
    setFormData({ 
      code: rowData.code, 
      name: rowData.name,
  ...
  const handleDelete = async (code: string) => {
      toast("Are you sure you want to delete this report group?", {
        action: (
          <button
            onClick={async () => {
              try {
                await deleteReportGroup(code);
                toast.success("Report Group deleted successfully!");
                setRefreshKey(prev => prev + 1);
              } catch (error) {
                toast.error("Error deleting report group.");
              }
            }}
  ...
  const handleSave = async () => {
    try {
      if (editData) {
        ...
        await addReportGroup(
          formData.code, 
          formData.name,
          formData.accountcode,
          formData.inactive);
        toast.success("Report Group added successfully!");
      }
      setShowAddForm(false);
      setEditData(null);
      setFormData({ code: '', name: '',accountcode:'',inactive:'0' });
    } catch (error) {
      toast.error("Failed to save report Group.");
    }
  };
            ...
            <ToolbarHeading>
              ...
              <ToolbarDescription>
                <div className="flex items-center flex-wrap gap-1.5 font-medium">
                  <span className="text-md text-gray-600">Report Group Management</span>
                </div>
              </ToolbarDescription>
            </ToolbarHeading>
            <ToolbarActions>
        ...
        {showAddForm ? (
          <div className="card">
            <div className="card-header" id="webhooks">
              <h3 className="card-title">{editData ? 'Edit Report Group' : 'New Report Group'}</h3>
            </div>
          ...
          <div className="overflow-x-auto ">
            <DataGrid
              ...
              columns={columns}
              serverSide={true}
              onFetchData={fetchReportGroup}
              rowSelection={true}
              pagination={{ size: pageSize }}
              toolbar={
                <div className="card-header flex-wrap gap-2 border-b-0 px-5">
                  <h3 className="card-title font-medium text-sm">
                    Showing {count} of {totalCount} report groups
                  </h3>
